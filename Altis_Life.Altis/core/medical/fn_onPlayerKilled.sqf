...
if ( _killer != _unit ) then {
  if (side _killer == west and playerSide == west ) then {  _multa_fogoamigo = 10000; BANK = BANK-10000;}   
    ...
    if (group player == group _killer) then {
     _multa_fogoamigo=0;
    };
  };
};
...
_unit spawn {
    ...
    if (LIFE_SETTINGS(getNumber,"respawn_timer") < 5) then {
    	...
    	_medicsOnline = {_x != player && {side _x isEqualTo independent} && {alive _x}} count playableUnits ;
		_maxTime = time + 20 + _medicsOnline * 120  ;
...
*/
    private["_maxTime","_requestBtn","_requestTime"];
    disableSerialization;
    _requestBtn = ((findDisplay 7300) displayCtrl 7303);
    _requestTime = ((findDisplay 7300) displayCtrl 7301);
...
if (_multa_medico > 0  or _multa_fogoamigo> 0  ) then {
	_killer setVariable ["multa_fogoamigo",_multa_fogoamigo,true];
	_killer setVariable ["multa_medico",_multa_medico,true];
	hint format["Seu assassino foi Multado: SemAnuncio %1 Rendido %2 Médico %3 FogoAmigo %4",_multa_sa,_multa_rendido,_multa_medico,_multa_fogoamigo];
};
